...
function reserveTable() {
    ...
    const reserveTime = document.getElementById('reserveTime').value.trim();
    ...
    if(table && !table.reserved){
        table.reserved = true;
        occupiedTables.push({number: table.number, customer: customerName, time: reserveTime});
        renderTables();
...
function generateReport() {
    ...
    if (occupiedTables.length === 0) {
        reportOutput.textContent = 'No hay mesas reservadas actualmente.';
        return;
    }
    let reportText = 'Reporte de Mesas Reservadas:\n\n';
    occupiedTables.forEach(table => {
        reportText += `Mesa ${table.number} - Cliente: ${table.customer} - Hora: ${table.time || 'No especificada'}\n`;
    });
